...
function CoursesBox() {
  ...
  return (
    <div className="course">
      ...
      <CourseFrom />
      <a
        onClick={authUser.gender === 'student' ? handleClick : openCourseFrom}
        style={{ cursor: 'pointer' }}
      >
        {authUser.gender === 'student' ? (
          < ReadMoreIcon style={{ color: "#674818", fontSize: '50px', marginLeft: '2rem', marginTop: '2rem' }} />
        ) : (
          <AddCircleIcon style={{ color: "#674818", fontSize: '50px', marginLeft: '2rem', marginTop: '2rem' }} />
        )}
      </a>
    </div>
  );
